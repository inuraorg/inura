...
```shell
# Build inura-program server-mode and MIPS-client binaries.
cd ../inura-program
make inura-program # build

# Switch back to cannon, and build the CLI
cd ../cannon
make cannon

# Transform MIPS inura-program client binary into first VM state.
# This outputs state.json (VM state) and meta.json (for debug symbols).
./bin/cannon load-elf --path=../inura-program/bin/inura-program-client.elf

# Run cannon emulator (with example inputs)
# Note that the server-mode inura-program command is passed into cannon (after the --),
