...
import (
	...
	"github.com/inuraorg/inura/inura-e2e/e2eutils/wait"
	...
	"github.com/inuraorg/inura/inura-service/client"
	"github.com/inuraorg/inura/inura-service/eth"
	oppprof "github.com/inuraorg/inura/inura-service/pprof"
	"github.com/inuraorg/inura/inura-service/retry"
...
func TestRequiredProtocolVersionChangeAndHalt(t *testing.T) {
	...
	cfg.Nodes["verifier"].RuntimeConfigReloadInterval = time.Second * 5
	cfg.Nodes["verifier"].Driver.VerifierConfDepth = 1
	// configure halt in verifier inura-node
	...
	require.NoError(t, err)
	t.Log("verified that inura-node closed!")
	// Checking if the engine is down is not trivial in inura-e2e.
	// In op-geth we have halting tests covering the Engine API, in inura-e2e we instead check if the API stops.
	_, err = retry.Do(context.Background(), 10, retry.Fixed(time.Second*10), func() (struct{}, error) {
