...
func applyCannonConfig(
	...
	c.CannonServer = "../inura-program/bin/inura-program"
	c.CannonAbsolutePreState = "../inura-program/bin/prestate.json"
...
func NewChallengerConfig(t *testing.T, l1Endpoint string, options ...Option) *config.Config {
	...
	require.NoError(t, cfg.Check(), "inura-challenger config should be valid")
	...
	if cfg.CannonServer != "" {
		...
		require.NoError(t, err, "inura-program should be built. Make sure you've run make cannon-prestate")
	}
