...
var (
	...
	cannonServer            = "./bin/inura-program"
...
func TestCannonServer(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		cfg := configForArgs(t, addRequiredArgsExcept(config.TraceTypeCannon, "--cannon-server", "--cannon-server=./inura-program"))
		require.Equal(t, "./inura-program", cfg.CannonServer)
	})
}
...
func runWithArgs(cliArgs []string) (log.Logger, config.Config, error) {
	...
	fullArgs := append([]string{"inura-challenger"}, cliArgs...)
