...
target "inura-node" {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-node:${tag}"]
}

target "inura-batcher" {
  dockerfile = "Dockerfile"
  context = "./inura-batcher"
  args = {
    OP_STACK_GO_BUILDER = "op-stack-go"
  }
  contexts = {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-batcher:${tag}"]
}

target "inura-proposer" {
  dockerfile = "Dockerfile"
  context = "./inura-proposer"
  args = {
    OP_STACK_GO_BUILDER = "op-stack-go"
  }
  contexts = {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-proposer:${tag}"]
}

target "inura-challenger" {
  dockerfile = "Dockerfile"
  context = "./inura-challenger"
  args = {
    OP_STACK_GO_BUILDER = "op-stack-go"
  }
  contexts = {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-challenger:${tag}"]
}

target "inura-heartbeat" {
  dockerfile = "Dockerfile"
  context = "./inura-heartbeat"
  args = {
    OP_STACK_GO_BUILDER = "op-stack-go"
  }
  contexts = {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-heartbeat:${tag}"]
}

target "inura-program" {
  dockerfile = "Dockerfile"
  context = "./inura-program"
  args = {
    OP_STACK_GO_BUILDER = "op-stack-go"
  }
  contexts = {
  ...
  platforms = split(",", PLATFORMS)
  tags = [for tag in split(",", IMAGE_TAGS) : "${REGISTRY}/${REPOSITORY}/inura-program:${tag}"]
}
